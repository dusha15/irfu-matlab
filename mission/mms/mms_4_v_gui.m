function out=mms_4_v_gui(x1,x2,x3,x4,column)
%mms_4_v_gui interactive discontinuity analyzer for MMS (for CLUSTER use c4_v_gui)
%
%  mms_4_v_gui(x1,x2,x3,x4,column)	use interactive discontinuity analyzer 
%		on variables x1..x4 using column number 'column'
%  mms_4_v_gui('B?',column)			use variables B1,B2,B3,B4 from base workspace
%  mms_4_v_gui('B?')					use column 2
%  H = mms_4_v_gui(..)				return axis handles
%
% See also: C_4_V

%%%%% we have changed the call c_v to the mms version mms_v
...
%		mms_4_v_gui('dt');
				...
				for j_col=2:size(ud.var1,2)
					if j_col<=length(ud.hcol),
						if ishandle(ud.hcol(j_col)),
							...
							eval_str=['ud.hcol(j_col)=uimenu(ud.columns,''label'',''' num2str(j_col) ''',''callback'',''mms_4_v_gui(''''c' num2str(j_col) ''''')'');'];
							eval(eval_str);
						end
					else
						eval_str=['ud.hcol(j_col)=uimenu(ud.columns,''label'',''' num2str(j_col) ''',''callback'',''mms_4_v_gui(''''c' num2str(j_col) ''''')'');'];
	...
	case 'initialize'
		...
		irf_legend(0,['mmm\_4\_v\_int() ' datestr(now)],[0.01 0.99],'fontsize',7); % add information to the plot
		...
		uimenu('label','Auto &YLim','accelerator','y','callback','mms_4_v_gui(''autoY'')');
		uimenu('label','&Distance','accelerator','d','callback','mms_4_v_gui(''distance'')');
		uimenu('label','Click&Times','accelerator','t','callback','mms_4_v_gui(''click_times'')');
		uimenu('label','New&Variable','accelerator','v','callback','mms_4_v_gui(''new_var_enter'')');
	...
	case 'click_times'
		...
		if ud.ic==0,
			set(gcf,'windowbuttondownfcn', 'mms_4_v_gui(''click_times'')');
			ud.ic=1;
		else
